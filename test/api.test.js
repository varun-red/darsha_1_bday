...
test('public event payload includes computed fields', async () => {
  ...
  assert.ok(Array.isArray(event.milestones) && event.milestones.length > 0);
...
test('settings can be updated and validated', async () => {
  const bad = await json('PUT', '/api/admin/settings', { milestones_json: '{not json' });
  assert.equal(bad.status, 400);
  const good = await json('PUT', '/api/admin/settings', { child_name: 'Darsha ✨', venue_name: 'Fern Hollow', milestones_json: JSON.stringify([{ month: 'Month 1', title: 'Hello', detail: 'World' }]) });
  assert.equal(good.status, 200);
  const ev = await (await fetch(`${base}/api/event`)).json();
  assert.equal(ev.event.child_name, 'Darsha ✨');
  assert.equal(ev.event.milestones.length, 1);
});
